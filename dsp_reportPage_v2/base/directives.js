...
rgDirectives.directive('rgList', function() {
	return {
		...
		transclude: true,
		templateUrl: '/base/directives/rgList/rgList.tmpl',
		controller: function($scope) {
			...
			ctrler.list = s.list;
				// 选中项
			ctrler.selectedItem = null;

				// 选中其中一项
			ctrler.selectItem = function(item) {
				ctrler.selectedItem = item;
			};

			// 判断项是否为选中项
			ctrler.isSelected = function (item) {
				if (item === ctrler.selectedItem) {
					return true;
				}
				return false;
			};
			...
			// 移除某个元素
			ctrler.removeItem = function(item) {
				var list = ctrler.list;
				if ((index = list.indexOf(item)) >-1) {
					// 删除
					list.splice(index, 1);
					return true;
			...
			// 清空列表
			ctrler.clear= function() {
				ctrler.selectedItem = null;
				ctrler.list.length = 0;
			}

			angular.extend(s, s.ctrler);
		},
		compile: function(tElement, tAttrs) {
			return function(s, element, attrs, transclude) {
